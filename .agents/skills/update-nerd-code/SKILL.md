---
name: update-nerd-code
description: Improve `.agents/skills/nerd-code-local/SKILL.md` from reviewed feedback while keeping the core `nerd-code` skill read-only. Use as the outer self-improvement loop for Nerd Code.
---

# Update Nerd Code

Use this skill to turn real feedback into a minimal, reviewable diff for
`.agents/skills/nerd-code-local/SKILL.md`.

The core skill `.agents/skills/nerd-code/SKILL.md` is the stable cross-repo contract
...
- `.agents/skills/nerd-code/SKILL.md`
...
2. Read feedback records where `skill == "nerd-code"`.
...
6. Propose the smallest edit to `.agents/skills/nerd-code-local/SKILL.md` that prevents the observed failure without duplicating existing guidance.
7. Run the Skill validator:

```bash
python3 .agents/skills/skill-anatomy-optimizer/scripts/audit_skill.py .agents/skills/nerd-code-local/SKILL.md
```
